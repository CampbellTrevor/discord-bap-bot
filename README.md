...
Queue entries show **Checking**, **Matched**, **Unavailable**, or **Retry pending** as their playback metadata is verified. The worker checks one entry at a time, with a pause between checks, prioritizing songs near the front and yielding to playback preparation and requests. Confirmed unavailable entries stay visible until playback reaches them, then are skipped together without opening their audio. Temporary network errors, provider restrictions on the host, and rate limits back off and retry; they do not mark a song permanently unavailable. Existing saved queues are checked after restart.
