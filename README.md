...
- Website song searches show up to five results in YouTube and Spotify tabs. Choose **Add** on a result to queue that recording; searching alone never queues a song. Single requests are checked for a playback match before acceptance. Playlist links import a batch whose playback matches are checked in the background. Spotify selections use YouTube for audio.
...
These checks verify metadata, recording matches, and advertised audio availability. **Matched** does not mean audio has been buffered or guarantee that a provider will still serve it later. Successful public YouTube matches are saved with the queue and reused for ten minutes; stale matches are rechecked before a new search. Signed audio URLs are never persisted.
...
`AUDIO_CACHE_DIR` selects the private directory for temporary audio downloads and defaults to `DATA_DIR/.audio-cache`. Use disk storage: the VM Compose service mounts `/var/lib/bap-bot/audio-cache` at `/var/cache/bap-bot`, separate from queue/session storage and the 128 MiB `/tmp` tmpfs. `AUDIO_DOWNLOAD_TIMEOUT_MS` defaults to `90000`; accepted values are `10000` through `120000` milliseconds.
...
Radio retains the seed's validated YouTube recording ID for recommendation discovery, including when started from the current song. Expiring the ten-minute playback-match cache does not force the station to search for its seed again. Normal audio preparation still rechecks expired playback metadata. If a Spotify search with extra terms such as “official audio” returns no acceptable candidates, matching tries one title-and-artist-only search with the same artist, duration, and recording checks and the existing validation limit.
...
Temporary audio is limited to 128 MiB per file, 512 MiB total, and at most 16 files across active playback and preparation. The worker reserves capacity before downloading and removes files when their source ends or is discarded. These files are disposable; they do not belong in queue/session backups.

The worker prepares the next two waiting songs during the final two minutes of the current track. Each completed download gets its playable resource and a buffer of up to 150 Opus packets (about three seconds at Discord's packet cadence); at least 25 packets are prepared before it is marked ready. Compatible Opus sources avoid FFmpeg conversion, and other formats are converted ahead of the transition. Playback reuses that resource without adding end padding. Silence in the recording and Discord transport timing can still produce an audible gap.
...
At most two speculative sources are held across all servers. Completed local preloads remain reusable while the queue owns them, even through an hour-long preceding song or a long pause. Other streaming adapters retain a five-minute expiry and can prepare again after resume. Playback has priority; queue changes discard stale sources. The preloader looks past confirmed unavailable entries and failed preparation attempts to prepare the next playable songs, retaining already buffered sources. Temporary preparation failures impose a cooldown to avoid sweeping through the queue during a provider outage; a missing preload can still require a download at playback time. Validated Spotify-to-YouTube matches are cached for ten minutes, up to 200 entries, so canceled preparation does not repeat the same search. Audio URLs are extracted afresh for each download. Matching recognizes translated titles and release credits while retaining artist, duration, and studio-version checks.
...
History aggregation, disk encoding, and performance-response encoding yield between small batches so a full day of statistics does not block the audio sender in one long operation. Saves reuse encoded minute buckets until those readings change. Snapshots and atomic saves retain a consistent point in time even as new playback measurements arrive.

For mid-song dropouts, the panel also records empty audio-buffer reads, minimum buffered packets, read-cadence peaks, Opus packet duration/size checks, voice connection changes, and voice WebSocket ping. Event-loop delay is sampled at 20 ms resolution and summarized every five seconds. Intentional pauses and song changes are excluded from read-cadence gaps; an empty read at normal EOF is distinguished from starvation. These diagnostics retain numeric summaries only.

The worker also emits structured `Voice diagnostics.` logs: connection/network state changes, WebSocket close codes, socket errors, and a summary every 30 seconds. Event bursts are capped. Summaries include audio-buffer health, outgoing UDP audio packet timing, the local socket send queue, voice WebSocket heartbeat age, and round-trip times for matching replies to the library's existing UDP keepalives. The observer sends no additional probes and does not retain audio, handshake payloads, tokens, user names, or song titles. Server/channel IDs identify the affected connection. Docker keeps up to three 10 MiB log files; container recreation replaces these logs, so capture them before redeploying during an incident.

UDP keepalive measurements are also saved in the 24-hour numeric performance history and returned by the manager-only performance endpoint under `network`. The installed voice library does not maintain UDP ping itself, so the observer correlates existing keepalive counters with replies. Unknown or stale RTT remains `null`; `udpKeepaliveConfirmed` distinguishes a socket that has returned a matching reply from one that has not. `udpKeepaliveTimeouts` counts unanswered keepalives after 15 seconds, **not lost audio packets**. Send counters measure local handoff, not delivery. These diagnostics can distinguish local starvation, scheduling delays, connection resets, and keepalive anomalies; they cannot establish packet loss between Discord and individual listeners.

During a dropout, note the time and read the worker logs, for example `sudo docker logs --since 30m --timestamps bap-bot-worker-1`. Correlate the diagnostic summaries with CPU/steal, memory, event-loop, and buffer readings for the same window before restarting. Avoid enabling raw library debug logging, which may include connection credentials.

### Developer activity

Set `DEVELOPER_DISCORD_USER_ID` on the audio worker (or combined host) to the numeric Discord user ID of the single account allowed to view all-server command activity. Access is disabled when unset. The portal obtains this permission from the worker using the signed-in Discord identity; server management permissions and usernames do not grant developer access.

That account sees **Developer** in the portal, with server, user, source, and result filters. Records include Discord slash commands and intentional web actions such as searching, requesting songs, playback controls, volume changes, and radio. Each completed action records its time, user/server identity, bounded parameters, result, and duration. Routine queue polling, authentication traffic, and developer reads are excluded. Commands rejected before reaching the bot, and commands still running when the process is forcibly killed, are not a complete audit trail. History starts when this feature is deployed; earlier commands cannot be recovered.

The worker retains up to 50,000 records for 30 days in `DATA_DIR/.command-activity.jsonl`, surviving container recreation through the existing persistent volume. Writes are batched asynchronously to avoid delaying playback; a crash can lose recent unflushed entries. URL tracking parameters, arbitrary URLs, credentials, headers, and raw error messages are not copied into the journal. Storage failures do not stop music, and the view reports dropped records when its write buffer cannot accept them.

The configured developer can also view **PLAYBACK HOST** statistics regardless of the selected server's role permissions. Other accounts still need the existing server manager or DJ permission.
